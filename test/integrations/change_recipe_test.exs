defmodule LetzellWeb.Integrations.ChangeRecipeTest do
  ...
  describe "when user edit a recipe" do
    test "with successful", %{session: session, user: user} do
      ...
      recipe = Letzell.Recipes.Recipe |> Repo.get(recipe.id)
  ...
  test "when user delete a recipe", %{session: session, user: user} do
    ...
    assert Letzell.Recipes.Recipe |> Repo.get(recipe.id) == nil
  end
end
