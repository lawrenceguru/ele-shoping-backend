defmodule Letzell.PostalCode.Navigator do
  use GenServer
  alias :math, as: Math
  alias Letzell.PostalCode.{Store, Cache}
  ...
  defp do_get_distance(from, to) do
    ...
    case Cache.get_distance(from, to) do
      nil ->
        from_location = get_geolocation(from)
        to_location = get_geolocation(to)

        lat1 = from_location.latitude
        long1 = from_location.longitude

        lat2 = to_location.latitude
        long2 = to_location.longitude

        # {_place1, _state1, lat1, long1} = get_geolocation(from)
        # {_place2, _state2, lat2, long2} = get_geolocation(to)
