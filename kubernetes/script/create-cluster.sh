...
gcloud config set project letzell-phoenix-graphql
...
NAMESPACE=letzell-staging
INGRESS_IP=$(kubectl -n istio-system get service istio-ingressgateway -o jsonpath='{.status.loadBalancer.ingress[0].ip}')

# Create namespace
kubectl create namespace $NAMESPACE
kubectl label namespace letzell-staging istio-injection=enabled
...
if [ -z "${HOST}" ]; then 
  ...
  if [ ! -e transaction.yaml ]; then
    gcloud dns record-sets transaction start --zone letzell-phoenix-graphql
  fi
  gcloud dns record-sets transaction add --zone letzell-phoenix-graphql --name $HOST. --ttl 300 --type A $INGRESS_IP
  gcloud dns record-sets transaction execute --zone letzell-phoenix-graphql
  gcloud dns record-sets transaction start --zone letzell-phoenix-graphql
  gcloud dns record-sets transaction remove --zone letzell-phoenix-graphql --name $HOST. --ttl 300  --type A $INGRESS_IP
fi
...
helm install --wait --name letzell --namespace=$NAMESPACE \
  --set host=$HOST_OR_IP \
...
kubectl exec -it $API_POD -- /opt/app/bin/letzell migrate

# Populate database
kubectl exec -it $API_POD -- /opt/app/bin/letzell seeds
